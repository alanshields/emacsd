...
(defun update-should-untabify (symbol value)
  (if value
      (add-hook 'write-file-hooks 'untabify-before-write)
    (remove-hook 'write-file-hooks 'untabify-before-write))
  (set-default symbol value))

(defcustom should-untabify t
  "If a true value then all files not in `untabify-exclude-list' will have
all tabs removed on write."
  :initialize 'update-should-untabify
  :set 'update-should-untabify
  :group 'untabify)
